...
> 提示：strong、em、ins、del 标签自带强调含义（语义）。 （工作常用）
## 10-图像标签
作用：在网页中插入图片
~~~ html
<img  src="图片的 URL">
...
> 1. 特殊的绝对路径 → 文件的在线网址：<https://www.itheima.com/images/logo.png> "，应用场景：网页底部**友情链接**
...
1. 明确合并的目标
1. 保留**最左最上**的单元格，添加属性（取值是**数字**，表示需要**合并的单元格数量**）
...
~~~
### 有语义的布局标签
![image-20240704184002314](file:///C:/Users/86138/AppData/Roaming/Typora/typora-user-images/image-20240704184002314.png)
...
1. 如果是错误的属性，有黄色叹号
1. CSS 属性的前面有多选框，如果勾选：这个属性生效； 如果没有勾选：这个属性不生效
...
* 必须设置 **content: ””属性**，用来 设置伪元素的内容（否则伪元素不生效），如果没有内容，则**引号留空**即可
* 伪元素默认是**行内**显示模式
* **权重和标签选择器相同**
## 02-PxCook
...
> **记忆技巧**：从**上**开始**顺时针**赋值，当前方向没有数值则与**对面取值相同**。
### 尺寸计算
...
属性值：X 轴偏移量  Y 轴偏移量  模糊半径 （柔和程度） 扩散半径 （阴影大小） 颜色  内外阴影
...
1. 设置盒子**背景图**为精灵图
1. 添加 **background-position** 属性，改变**背景图位置**
